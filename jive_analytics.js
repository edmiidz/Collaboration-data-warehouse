...
function jiveActivity()
{
    ...
    var query = "SELECT `ActivityTime` FROM " + config.tables.activity + " ORDER BY `ActivityTime` limit 1"; 
    ...
    connection.query(query, (err,rows) => {
        if (!err) {
            ...
            var before_date = current_date;
            var last_time = dateFormat(new Date('1970-01-01 00:00:00'), "%H:%M:%S", true);

            if(rows.length > 0) {
                before_date = moment(rows[0]['ActivityTime']).format('YYYY-MM-DD');
                console.log("from DB: "); 
                console.log(before_date + 'T' + last_time);             
            } 
            else{
            			    ...
            			    if (String(jive_ans.code) == '401'){
                                ...
                                async.eachSeries(jive_ans.list, function iterator(rows_1, callback2) {
                                    ...
                                    insertMultipleRows(queries).then(function(res1) {
                                        ...
                                        maximum_rows += 300;
                                        if(typeof jive_ans.paging.next == 'undefined' || maximum_rows >= 2700000) {
                                            next_page = false;
                                        } else {
...
{
    var deferred = q.defer();
    var ja_auth_url = jaconfig.ja_basic_url + "/analytics/v1/auth/login?clientId=" + jaconfig.ja_client_id + "&clientSecret=" + jaconfig.ja_client_secret;
